...
def _get_data(data_url, problem):
    ...
    X = problem.get_preprocessor(data).fit_transform(
...
def benchmark_deep_learning(problem: CIFARProblem,
                            time_budget_s: float,
                            max_concurrent: int = 1,
                            random_seed: int = 1,
                            searcher_name: Optional[str] = None,
    ...
    for name, searcher in searchers.items():
        if not (searcher.supports_categorical and searcher.suitable_for_dl):
            if searcher_name is not None:
                raise ValueError(
                    f"{searcher_name} doesn't support categorical variables or is not suitable for dl."
                )
            continue
        ...
        search_alg = searcher.get_searcher_instance(config,
                                                    ...
                                                    max_concurrent)
        name = (f"{problem.__class__.__name__}-{searcher.__class__.__name__}"
                f"-{random_seed}-{time_budget_s}-{max_concurrent}")
        results_path = "~/results"
        run_on_every_ray_node(set_up_s3fs, path=results_path)
        results_path_expanded = os.path.expanduser(results_path)
        save_path = os.path.join(results_path_expanded,
